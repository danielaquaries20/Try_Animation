...
public class DateRangeCalendarView extends LinearLayout {
    ...
    public void setMinMax(Calendar min, Calendar max) {
        calendarStyleAttr.setEnabledDisableDate(true);
        calendarStyleAttr.setMinDate(min);
        calendarStyleAttr.setMaxDate(max);
    }
     ...
     */
    public void setVisibleMonthRange(Calendar startMonth, Calendar endMonth, Boolean startFromEnd) {

        if (startMonth == null) {
            throw new IllegalArgumentException("Start month can not be null.");
        }
        Calendar montStart = (Calendar) startMonth.clone();
        ...
        Calendar monthEnd = (Calendar) endMonth.clone();
        ...
        calendarStyleAttr.setMinDate(montStart);
        calendarStyleAttr.setMaxDate(monthEnd);
        calendarStyleAttr.setEnabledDisableDate(true);
        ...
        do {
            ...
            if (startMonth.before(endMonth)) { //Calendar.getInstance()
                currentDay += 1;
            }
        }
        ...
        vpCalendar.setOffscreenPageLimit(0);
        if (startFromEnd) {
            vpCalendar.setCurrentItem(currentDay);
            setCalendarYearTitle(currentDay);
            setNavigationHeader(currentDay);
        } else {
